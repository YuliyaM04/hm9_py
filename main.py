...
def getToken():
    ...
    if os.path.isfile(st.BOT_TOKEN_FILENAME):
        ...
        print('создайте файл и поместите туда токен')
        sys.exit()
    return token

def isWin(arr, who): #arr massiv, who - (x and o)
...
def countUndefinedCells(cellArray):  #неопеределенные ячейки
    counter = 0
    for i in cellArray:
...
def game(callBackDate):
    message = st.ANSW_YOUR_TURN
    alert = None
    buttonNumber = int(callBackDate[0])
    if not buttonNumber == 9:
        charList = list(callBackDate)
        charList.pop(0)
